...
def approve_user(email):
    ...
    user_to_approve, error = AdminService.approve_user(email)
    
    if error:
        logger.warning(f"User approval failed - {error}")
...
def deny_user(email):
    ...
    user_to_deny, error = AdminService.deny_user(email)
    
    if error:
        logger.warning(f"User denial failed - {error}")
        return jsonify({"error": error}), 404
...
def re_pend_user(email):
    ...
    user_to_re_pend, error = AdminService.re_pend_user(email)
    
    if error:
        logger.warning(f"Re-pend failed - {error}")
        return jsonify({"error": error}), 404
...
def toggle_role(email):
    ...
    updated_user, error = AdminService.toggle_user_role(email)
    
    if error:
        logger.error(f"Toggle role failed - {error}")
        return jsonify({"error": error}), 404
...
def toggle_status(email):
    ...
    updated_user, error = AdminService.toggle_user_status(email)
    
    if error:
        logger.error(f"Toggle status failed - {error}")
        return jsonify({"error": error}), 404
